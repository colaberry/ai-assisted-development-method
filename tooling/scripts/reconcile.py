...
Assumptions (tune for your team's conventions):
  ...
  - Tasks in TASKS.md use checkbox format:
    ...
    "Target: vN" line.
  - The "Files:" subline lists comma-separated paths. The script greps these
    files for any symbol mentioned in the Satisfies: IDs to verify presence.
...
def determine_confidence(task: Task, existing_files: list[str]) -> str:
    """
    Rough heuristic for confidence:
      HIGH   — task is complete, has Satisfies: IDs, and all listed files exist
      MEDIUM — task is complete, has Satisfies: IDs, but some listed files are missing
      LOW    — task is complete but has no files listed, or files are empty
    """
...
def build_coverage(
    ...
    for req in requirements:
        ...
        if completed:
            ...
            confidence = determine_confidence(task, existing)
            entries.append(
                CoverageEntry(
                    ...
                    notes=(
                        f"Listed {len(task.files)} file(s); "
                        f"{len(existing)} found"
                        if task.files
                        else "No files listed on task"
                    ),
                )
            )
        elif deferred:
...
def main() -> int:
    ...
    entries = build_coverage(requirements, tasks, repo_root)
